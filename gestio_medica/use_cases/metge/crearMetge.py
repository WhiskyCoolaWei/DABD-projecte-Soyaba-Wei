...
def crearMetge(dni, nom, cognoms, data_naixement, telefon, adreca, correu, num_collegiat):
    if Persona.objects.filter(dni = dni).exists():
       ...
       raise ValueError(f"L'usuari amb DNI {dni} ja existeix en la BBDD.")
    
    if len(dni) != 9:
        raise ValueError(f"DNI invalid: Mida incorrecta")
    
    if Metge.objects.filter(num_collegiat = num_collegiat).exists():
        raise ValueError(f"Número de collegiat ja existent en la BBDD")
    
    #Esta comprovacion es un poco rara pero bueno, prevenimos 
    #fechas futuras por lo menos
    if data_naixement > date.today():
        raise ValueError(f"Data de naixement invalida.")


#Creamos nueva persona y luego le asignamos numero de colegiado para que sea medico

    npersona = Persona.objects.create(
        dni = dni,
        nom = nom,
        cognoms = cognoms,
